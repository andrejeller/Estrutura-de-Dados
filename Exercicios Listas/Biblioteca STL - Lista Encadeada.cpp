...
//funçao para incluir um novo jogador ao registro
void incluir(vector<jogador>* registro) {
	///// Incluir
	jogador inclusao;
	qnt++;
...
//função para excluir um jogador do registro
void excluir(vector<jogador>* registro) {
	bool excluir = false;
	char yes;

	//Apenas continua para telade exclusão caso exista algum jogador na lista
	if (registro->size() > 0) {
		while (!excluir) {
			...
			//Caso não exista nada na posição escolhida
			if (posicao > registro->size())
				std::cout << "Jogador inexistente, tente novamente." << std::endl;
			
			//Se realmente existe um jogador na posição escolhida
			else {
...
//função para consultar um jogador
void consultar(vector<jogador>* registro) {
	int opcao, posicao;
...
//A função de rearranjar os jogadores Presente na Lista Contigua não existe 
